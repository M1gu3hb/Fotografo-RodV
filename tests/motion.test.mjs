...
test('typography and motion system are explicit, responsive and reduced-motion safe', () => {
  ...
  assert.match(app, /MotionText/);
  assert.match(app, /StoryRail/);
  assert.match(app, /collection-showcase/);
  ...
  assert.match(app, /<MotionText as="h1" text=\{currentCollection/);
});

test('brand motion and collection layout stay semantic', () => {
  const app = readFileSync('src/App.tsx', 'utf8');
  assert.match(app, /brand__motion/);
  assert.match(app, /aria-label="Colecciones fotográficas"/);
  assert.ok(app.includes('href={`/colecciones/${collection.slug}`}'));
});
